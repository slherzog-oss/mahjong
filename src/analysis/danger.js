...
export function opponentTempo(state, seat) {
  ...
  if (p.riichi) return 1; // Riichi: die Hand ist wartend
...
export function dangerOf(state, seat, kind, remaining) {
  ...
  for (const p of state.players) {
    ...
    if (p.riichi) {
      // Riichi: eigene Abwürfe des Riichi-Spielers und alles seit dem Riichi Abgeworfene sind sicher (Furiten)
      const safe = p.discards.some((id) => kindOf(id) === kind) || p.riichi.safe.includes(kind);
      if (safe) continue;
      d += 0.35;
      // Suji: Nachbar im Abstand 3 ist sicher → beidseitiges Warten unwahrscheinlicher
      if (isSuited(kind)) {
        const safeSet = new Set([...p.discards.map(kindOf), ...p.riichi.safe]);
        const r = kind % 9;
        if ((r >= 3 && safeSet.has(kind - 3)) || (r <= 5 && safeSet.has(kind + 3))) d -= 0.1;
      }
      continue;
    }
