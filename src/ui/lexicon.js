...
function worth(entry) {
  const r = entry.rule ? SCORE_RULES[entry.rule] : null;
  if (!r) return '';
...
export function renderLexicon(ui) {
  const entries = lexiconEntries(ui.lexicon);
  const cat = ui.lexicon.category;
  const chips = ['all', ...CATEGORIES].map((c) => `<button class="chip${cat === c ? ' active' : ''}" data-action="lexicon-filter" data-category="${c}">${t('lex.' + c)}</button>`).join('');
...
export function renderFormsPanel(forms, { target, expanded, showKinds }) {
  const rows = forms.slice(0, 7).map((f) => {
    const name = t('forms.names')[f.form] ?? f.form;
  ...
  <details class="forms" ${expanded ? 'open' : ''}>
    <summary>${t('forms.title')}${target ? ` · ${t('forms.targetActive')}: ${esc(t('forms.names')[target] ?? target)}` : ''}</summary>
